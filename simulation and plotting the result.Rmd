---
title: "simulation and ploting the result"
output: pdf_document
---
```{r}
library(combinat)
...
########################################################################
#### the simulation part and the plotting for the result.

win_rate <- rerun(1000,rand_strategy())
win_matrix <- data.frame(matrix(unlist(win_rate), nrow=1000, byrow=T))
names(win_matrix) <- c('result','index')   #it is shown the result 1 if first player win,-1 second wins,and 0 if no one win. The index show which step the game ended by winning or no one win
